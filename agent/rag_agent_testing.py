...
def main():
    ...
    for chunk in graph.stream(
        ...
        for node, update in chunk.items():
            print("="*80)
            print(f"UPDATE FROM NODE: {node.upper()}")
            print("="*80)

            # Special handling for retrieve node to show segments clearly
            if node == "retrieve":
                ...
                if hasattr(last_message, 'content'):
                    ...
                    # Try to split by common separators used by retrievers
                    # The retriever tool typically separates docs with \n\n
