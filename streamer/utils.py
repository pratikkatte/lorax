...
def set_terminal_y_coords(root, start_num=0):
    number = start_num
    for i, node in alive_it(enumerate(root.traverse_leaves(), start=number),
...
def get_node_object(node, node_to_index, metadata, input_to_index, columns,
                    chronumental_enabled, mutations=None, tree_idx=0):

    object = {}
    object["name"] = node.label if node.label else ""
    object['tree'] = str(tree_idx)
    # round to 5 dp
