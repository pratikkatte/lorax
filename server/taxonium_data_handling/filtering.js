...
function getNodes(data, y_positions, min_y, max_y, min_x, max_x, xType,  useHydratedMutations = false,
  mutations = null, treeposition=[]) {
  console.log("GETNODES", min_y, max_y, min_x, max_x, xType, treeposition);
  const start_time = Date.now();
  // get min_x, max_x, min_y, max_y from URL
  data = treeposition.length > 0 ? data.filter(dict => {
    return treeposition.includes(dict['tree']);
  }) : data;
