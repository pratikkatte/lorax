...
import GenomeBrowserUI from './Slider'
...
function Taxonium({
  ...
  if (treeposition === null && config.tree_min_max){
    setTreeposition([config.tree_min_max.start , config.tree_min_max.end])
  }
  ...
  const { data, boundsForQueries, isCurrentlyOutsideBounds } =
    useGetDynamicData(backend, colorBy, view.viewState, config, xType, value);
  ...
  useEffect(() => {
    ...
    console.log("treeposition", value)
  ...
  return (
    <>
    <div className="fixed top-0 left-0 right-0 flex justify-center items-start w-full bg-white z-10">
      
      {treeposition && (
        <GenomeBrowserUI tree_min_max={treeposition} val={value} setVal={setValue}/>
      )}
