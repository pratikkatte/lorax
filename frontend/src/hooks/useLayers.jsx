...
import { inferno } from "scale-color-perceptual";
...
const useLayers = ({
  ...
  const layers = [];
     ...
     // Example genome positions
  // const genome_positions = [
  //   { x: 0-300, y: detailed_data.min_y, text: "0" },
  //   { x: 0-300, y: detailed_data.max_y, text: "100"},
  // ];

  const genome_positions = useMemo(() => {
    if (data.data && data.data.trees_position){
      const positions = []
      const ignore = new Set();

      console.log("dynamic ", data.data.tree_po)
      Object.entries(data.data.tree_po).forEach(([key, position]) => {
        if (!ignore.has(position.min_text)) {
          positions.push({x: 0 - 100, y: position.min_y, text: position.min_text});
          ignore.add(position.min_text);
        }
        if (!ignore.has(position.max_text)){
            positions.push({x: 0 - 100, y: position.max_y, text: position.max_text});
            ignore.add(position.max_text);
        }
        })
        

      // data.data.trees_position.forEach((position) => {
      //   positions.push({
      //     x: 0, y: position.y, text: position.text
      //   })
      // })
      return positions
    }
  ...
  console.log("genome_positions", genome_positions)

  const onHoverText = useCallback((info) => {
    // setHoverInfo(info);
    if (info.object) {
      console.log(`Hovered over: hello`,info );
      setHoveredText({
        position: [info.coordinate[0], info.coordinate[1]],
        text: `Tree: ${info.object.tree}`, // Replace 'someProperty' with the actual property you want to display
      });
    } else {
      setHoveredText(null);
    }
  ...
  const getX = useCallback((node) => {
    if (!node || !(xType in node)) {
      console.log(`getX`, node);
    }
    return node[xType];
  }, [xType])
  ...
  const line_layer_vert_common_props = {
    getSourcePosition: (d) => {
      return [d.parent_x, d.y];
    },
    getTargetPosition: (d) => {
      return [d.parent_x, d.parent_y];
    },
    onHover: (info) => setHoverInfo(info),
    getColor: lineColor,
    pickable: true,
  ...
  const genomic_inner_line_vert_common_props = {
    getSourcePosition: (d) => {
      return [0-50, d.y];
    },

    getTargetPosition: (d) => {
      return [0-50, d.parent_y];
    },
  ...
  if (detailed_data.nodes) {
    ...
    const main_inner_line_layer = {
      layerType: "LineLayer",
      ...genomic_inner_line_vert_common_props,
      id: "main-genomicline-vert",
      data: detailed_data.nodes
    }

    const hoverTextLayer = hoveredText ? {
      layerType: "TextLayer",
      id: "main-hover-text-layer",
      data: [hoveredText],
      getPosition: (d) => d.position,
      getText: (d) => d.text,
      getColor: [0, 0, 0], // Black color for the text
      getSize: 15, // Font size
      fontFamily: "Arial, sans-serif",
      getTextAnchor: "start",
      getAlignmentBaseline: "center",
      getPixelOffset: [10, 10], // Adjusts the margin around the text
      getBorderWidth: 1, // Border width in pixels
      getBorderColor: [0, 0, 0], // Black border color
      getBackgroundColor: [255, 255, 255, 200], // White background with some transparency

      billboard: true,
    } : null;

    const genome_text_layer = {
      layerType: "TextLayer",
      id: "main-genome-text-layer",
      data: genome_positions,
      modelMatrix: modelMatrix,
      getPosition: (d) => [d.x, d.y],
      getText: (d) => d.text,
      getColor: [0,0,0], // Black color for the text
      getSize: 12, // Font size
      fontFamily: "Arial, sans-serif",
      getTextAnchor: "start",
      getAlignmentBaseline: "center",
      billboard: true,
