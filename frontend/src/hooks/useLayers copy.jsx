...
const useLayers = ({
  ...
  // const getGenomeViewport = useCallback(() => {
  //   const deck = deckRef?.current?.deck;
  //   if (!deck) return null;
  //   return deck.getViewports().find(vp => vp.id === "genome-positions") || null;
  // }, [deckRef]);

  // const viewport = getGenomeViewport();
  ...
  const layers = useMemo(() => {
    ...
    const genomeGridLayer = new GenomeGridLayer({
      ...
      getText: d => d.start.toLocaleString("en-US", { maximumFractionDigits: 0 }),
    ...
    const genomeLineLayer_new = genomeGridLines?.length > 0 ? new LineLayer({
          id: `genome-positions-lines`,
          // data: [
          //   {sourcePosition: [0.0, 0], targetPosition: [0.0, 2], genomicPosition: 1},
          //   {sourcePosition: [2.5, 0], targetPosition: [2.5, 2], genomicPosition: 2}
          // ],
          data: genomeGridLines,
          getSourcePosition: d => d.sourcePosition,
          getTargetPosition: d => d.targetPosition,
    ...
    // getLayerDataWithingView(genomicLineLayer)
    // return [...singleTreeLayers,coalescenceLayer, coalescenceTimeLabels];
    return [coalescenceLayer, coalescenceTimeLabels, genomeGridLayer];

  }, [data, viewState.zoom, viewState['genome-positions'], hoverInfo, hoveredTreeIndex, settings, hideOrthoLayers]);
